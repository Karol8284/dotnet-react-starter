...
public class AuthControllerTests
{
    ...
    {
        _controller = new AuthController(_jwtTokenServiceMock.Object, _authServiceMock.Object, _loggerMock.Object);
    }
    ...
    public async Task Login_Returns_ok_when_credentials_are_valid()
    {
        ...
        var response = Assert.IsType<ApiResponse<JwtTokens>>(okResult.Value);
    ...
    public async Task Register_Returns_created_when_registration_succeeds()
    {
        ...
        var response = Assert.IsType<ApiResponse<JwtTokens>>(createdResult.Value);

        Assert.Equal(201, createdResult.StatusCode);
        Assert.Equal("Registration successful", response.Message);
        Assert.Equal(tokens.RefreshToken, response.Data?.RefreshToken);
    }
    ...
    public async Task RefreshToken_Returns_ok_when_refresh_succeeds()
    {
        ...
        _jwtTokenServiceMock.Setup(x => x.RefreshTokensAsync(request.RefreshToken)).ReturnsAsync(tokens);
    ...
    public async Task RefreshToken_Returns_unauthorized_when_refresh_token_is_invalid()
    {
        ...
        _jwtTokenServiceMock.Setup(x => x.RefreshTokensAsync(request.RefreshToken)).ReturnsAsync((JwtTokens?)null);
    ...
    public async Task Logout_Returns_ok_when_refresh_token_is_revoked()
    {
        ...
        _jwtTokenServiceMock.Setup(x => x.RevokeTokenAsync(request.RefreshToken)).Returns(Task.CompletedTask);
